...
parser.add_argument('--max_length', default='128', type=int, help='The maximum length')
...
def get_dataloader(news_text, test_label):
    ...
    test_encoding = tokenizer(news_text, truncation=True, padding=True, max_length=args.max_length)
...
def workflow(file_path, ind):
    ...
    print("%s开始执行,进程号为%d" % (ind, os.getpid()))
    #字段
    field = args.field
    # 加载数据
...
def main(file_path):
    ...
    for ind in range(0,int(lens/args.step)+1):
        ...
        po.apply_async(workflow, (file_path, ind,))
    ...
    print("-----end-----")
    t_stop = time.time()
    print("执行完毕，总耗时%0.2f" % (t_stop-t_start))
